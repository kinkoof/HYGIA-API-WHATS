...
exports.handleMessage = (req, res) => {
    ...
    if (messageObject.interactive?.type === 'button_reply') {
        handleButtonResponse(phone_number_id, from, messageObject.interactive.button_reply.id, res);
    } else if (messageObject.text) {
        handleTextMessage(phone_number_id, from, messageObject.text.body, res);
    } else if (messageObject.interactive?.type === 'list_reply') {
        handleListResponse(phone_number_id, from, messageObject.interactive.list_reply.id, res);
    }
};

// Processamento da resposta de botão
...
const handleTextMessage = (phone_number_id, from, userText, res) => {
    ...
    if (userFlows[from].status === 'awaiting_product') {
        ...
        handleCartResponse(phone_number_id, from, userText, res);
    } else {
        sendWelcomeOptions(phone_number_id, from, res);
    }
};

// Processamento da resposta de lista
...
// Tratar respostas do carrinho
const handleCartResponse = (phone_number_id, from, userText, res) => {
